...
public class RabbitTopicListener {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(RabbitConfig.TOPIC_QUEUE_NAME),
            exchange = @Exchange(value = RabbitConfig.TOPIC_EXCHANGE,type = ExchangeTypes.TOPIC),
            key = RabbitConfig.TOPIC_ROUTING_KEY
    ))
    public void getTopicMsg(Message message, Channel channel, String msg) throws IOException {
